 ...
 */
uint16_t
mlx5_mdev_tx_burst_mpw(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	struct mlx5_txq_data *txq = (struct mlx5_txq_data *)dpdk_txq;
	uint16_t elts_head = txq->elts_head;
	const uint16_t elts_n = 1 << txq->elts_n;
 ...
 */
uint16_t
mlx5_mdev_tx_burst_mpw_inline(void *dpdk_txq, struct rte_mbuf **pkts,
			 uint16_t pkts_n)
{
	struct mlx5_txq_data *txq = (struct mlx5_txq_data *)dpdk_txq;
	uint16_t elts_head = txq->elts_head;
 ...
 */
uint16_t
mlx5_mdev_tx_burst_empw(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	struct mlx5_txq_data *txq = (struct mlx5_txq_data *)dpdk_txq;
	uint16_t nb_tx = 0;
	...
	while (pkts_n > nb_tx) {
		...
		if (n) {
			ret = mlx5_mdev_tx_burst(dpdk_txq, &pkts[nb_tx], n);
			if (!ret)
				break;
			nb_tx += ret;
		}
...
mlx5_mdev_tx_burst_raw_vec(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	(void)dpdk_txq;
	(void)pkts;
	(void)pkts_n;
...
uint16_t __attribute__((weak))
mlx5_mdev_tx_burst_vec(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	(void)dpdk_txq;
	(void)pkts;
	(void)pkts_n;
...
priv_check_raw_vec_tx_support(struct mlx5_mdev_priv *priv, struct rte_eth_dev *dev)
{
	(void)priv;
	(void)dev;
	return -ENOTSUP;
...
int __attribute__((weak))
priv_check_vec_tx_support(struct mlx5_mdev_priv *priv, struct rte_eth_dev *dev)
{
	(void)priv;
	(void)dev;
	return -ENOTSUP;
...
int __attribute__((weak))
priv_check_vec_rx_support(struct mlx5_mdev_priv *priv)
{
	(void)priv;
	return -ENOTSUP;
}
